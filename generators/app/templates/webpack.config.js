...
function config(opts) {
   ...
   return {
      ...
      plugins: [
         ...
         // auto load modules
         new webpack.ProvidePlugin({<% if (options.react) { %>
      ...
      module: {
         ...
         loaders: [
            // js files
            {
               <% if (options.react) { %>test: /\.jsx?$/,
               <% } else { %>test: /\.js$/,<% } %>
               exclude: /(node_modules|bower_components)/,
               loader: <% if (options.react) { %>(DEV ? 'react-hot!' : '') + <% } %>'babel?optional[]=runtime&sourceMap'
            },
            ...
            // sass files
            {
               ...
               loader: ExtractTextPlugin.extract(
                  'style',<% if (options.react) { %>
                  // if you don't want CSS Modules, i.e. local scoped css by default,
                  // remove `modules` query param
                  'css?modules&sourceMap!postcss?sourceMap!sass?indentedSyntax&sourceMap' +<% } else { %>
                  // if you want CSS Modules, i.e. local scoped css by default,
                  // add `modules` query param
